    ...
    try {
        ...
        if(user) return res.status(500).send({success:false,message:"UserName or Email already exists"});
          const hashPassword=bcryptjs.hashSync(password,10);
           const profileBoy=profilepic || `https://avatar.iran.liara.run/public/28?username=${username}`;
           const profileGirl=profilepic || `https://avatar.iran.liara.run/public/99/?username=${username}`;
